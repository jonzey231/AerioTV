...
struct MyRecordingsView: View {
    ...
    // `PlayingRecording` lives in Features/DVR/RecordingActions.swift (shared with the DVR tab).
    ...
    /// Shared with the DVR tab: every action lives in `RecordingActions`.
    ...
    private func apiForRecording(_ rec: Recording) -> DispatcharrAPI? { actions.api(for: rec) }
    private func playIfCompleted(_ rec: Recording) { actions.play(rec) }
    private func playRecording(_ rec: Recording, path: String) { actions.playLocal(rec, path: path) }
    private func playServerRecording(_ rec: Recording, fromStart: Bool = false, liveEdge: Bool = false) {
        actions.playServer(rec, fromStart: fromStart, liveEdge: liveEdge)
    }
    private func stopRecording(_ rec: Recording) { actions.stop(rec) }
    private func cancelRecording(_ rec: Recording) { actions.cancel(rec) }
    private func deleteRecording(_ rec: Recording) { actions.deleteLocal(rec) }
    private func deleteFromServer(_ rec: Recording) { actions.deleteFromServer(rec) }
    private func downloadRecording(_ rec: Recording) { actions.download(rec) }
    private func runComskip(_ rec: Recording) { actions.runComskip(rec) }
}
