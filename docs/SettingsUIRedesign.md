...
Status as built, 2026-08-05. Three of five shipped; two were measured on a
1280dp tablet and deliberately not done, with the reasoning recorded at the
call site so they are not re-attempted blind.

- DONE. Tablet Appearance, theme swatch grid two-up (`487072c`). Gated on being
  IN A PANE, not on raw width: a phone in landscape is ~891dp and a width-only
  test would have reflowed it and broken the frozen phone canon.
  `LocalSettingsInPane` defaults false and is only set by the two hosts.
  TV is excluded explicitly even though its rail host sets the same flag and
  its pane clears the threshold, because two columns add a horizontal D-pad
  axis that competes with the rail boundary.
- DONE. Category palette two-up (`846a743`), same breakpoint, gate extracted to
  `twoUpInPane()` and the constant renamed `TwoUpMinPaneWidth`.
- NOT DONE, measured. The two display-scale sliders side by side. These are not
  thin sliders, they are inline percentage segment pickers
  (85/100/125/150/175), and the pane's content is capped at
  `Viewport.formMaxWidth` = 700dp regardless of window width, so each column
  gets ~330dp. Built it; the labels rendered one character per line. The
  threshold in this plan measured PANE width when the binding constraint is
  PER-SLIDER width, and no window size fixes it while the form cap stands.
  Raising that cap is a canon change affecting every settings screen.
  Logan reviewed and accepted the stacked layout.
- NOT DONE, measured. Denser two-line playlist rows on tablet. The row is
  already ~56dp against Material's 72dp two-line spec, so it is denser than
  standard already. The pane is also not height-constrained: the largest real
  lineup (8 playlists) is ~448dp inside ~660dp of usable pane, so it fits
  without scrolling. Compressing further buys no visible space and would add a
  phone/tablet divergence in an otherwise shared row.
- DONE. Motion: 150ms crossfade on pane content swaps, both hosts (`c2ef308`).
  Deliberately short because the rail commits its selection through a ~150ms
  debounce of its own; a longer fade still runs when a fast scroll reaches the
  next row and the pane visibly trails the highlight. The `Crossfade` wraps the
  `SaveableStateProvider` so per-pane scroll retention still keys off the
  rendered route.
- DONE. ManageGroupsSheet capped at 640dp and centred on tablet (`6f271bb`).
  `FormFactorModal` gained an OPT-IN `sheetMaxWidth` rather than a default,
  because every modal shares that container and defaulting it would have
  quietly narrowed the player sheets, Record from Now and What's New. The cap
  is on the content column, not the sheet, so the scrim and swipe-to-dismiss
  still span the window. The TV centered-dialog path is unchanged.
...
5. **Density polish** (B5). Effort S to M (about 2 days). DONE 2026-08-05, see
   the B5 status notes above for the two items measured and rejected.
6. **ManageGroups adaptive pass and cleanup** (retire `AdaptiveCenteredContent`, docs). Effort S (about 1 day).
   DONE 2026-08-05. `AdaptiveCenteredContent` removed (`c2ef308`): it had zero
   call sites, only its own definition and one doc reference. A breadcrumb at
   the old site points at `SettingsPaneContent`, which measures the PANE rather
   than the window. NOTE `adaptiveFormWidth` is still live in several settings
   screens and is a separate migration, NOT covered by this phase.

### Android phases, additional fixes found while building

Two defects surfaced during B4 device testing that were not in the plan:

- Edit Playlist could overwrite the WRONG playlist (`c47dd16`). The screen read
  `state.playlist` while `saveEdits` wrote `existingId = active.id`. Latent and
  unreachable until this redesign made non-active playlist details openable
  from the rail. Edit is now gated to the active playlist. Data-loss class; do
  not regress the gate.
- Add Playlist wizard titles sat inside TV overscan (`649cfe1`). Hiding the
  back arrow on TV collapses the Material `TopAppBar` navigation-icon slot and
  drops the title onto the default 16dp inset. `CenterAlignedTopAppBar` is
  immune, which is why Add More Categories looked right and the wizard did not.
  The 48dp overscan constant now lives in `TvSettingsMetrics.overscanStart`;
  `topAppBarTitleOverscanPadding` subtracts Material's own additive 16dp.

Pane navigation polish, also from device testing (`9699737`, `cbe0e29`):
Back from a pane-level push returns focus to the SIDEBAR's selected row
(Logan's call) instead of falling through to the top tab bar, and each pane
retains its scroll offset across a full-screen takeover via a
`SaveableStateHolder` hoisted above the takeover early-return.
